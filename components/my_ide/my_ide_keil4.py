...
class my_ide_keil4(my_ide_base):
    ide_kind = 'keil4'
    uvprojx_path = ''
    uv4_path = ''
    insert_group_num = 0   
    ...
    def tmake(self):
        ...
        KEIL4_PATH = my_exe_get_install_path('$KEIL4_PATH')   
        self.uv4_path = KEIL4_PATH+'/UV4'
    ...
    def _tlib(self,libs_path,incs_path,comp_path,log_path):
        ...
        for k,v in self.output['sdk']['components'].items():
            if k in libs:
                ...
                sdk_uvprojx_path = '../.log/SdkDemo.uvproj'
                my_file_copy_file_to_file('../.log/Demo.uvproj',sdk_uvprojx_path)
                ...
                cmd = 'UV4.exe -j0 -b SdkDemo.uvproj'
    ...
    def _create_subgroup(self,KIND,LIST,GROUP_NAME):
        ...
        if self.uvprojx_path == '':
            # copy keil to output
            keil4_path         =  self.cmd['bin_path'][1:] # ../vendor -> ./vendor
            build_path        =  '.log'
            my_file_copy_dir_contents_to(keil4_path,build_path)
        
            self.uvprojx_path =  build_path+'/Demo.uvproj' 
